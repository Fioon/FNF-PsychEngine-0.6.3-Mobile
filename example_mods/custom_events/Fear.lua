...
function onUpdate()
    ...
    for i = 0,getProperty('eventNotes.length')-1 do
        if math.abs((getPropertyFromGroup('eventNotes', i,'strumTime') - getSongPosition())) < 600 and getPropertyFromGroup('eventNotes', i,'event') == 'Fear' and not MickeyDodge then
            WarningMickey()
        end
    end
    ...
    if MickeyDodge == true and keyJustPressed(' ') then
        MickeyDodgeWow()
    end
